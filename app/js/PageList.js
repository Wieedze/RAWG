...
// Fonction pagination "Afficher plus"
...
// Intersection Observer pour afficher les cards au scroll
...
const PageList = (argument = '') => {
    ...
    function fetchGamesPage(page) {
        const urlBase = `https://api.rawg.io/api/games?key=${API_KEY}&ordering=-added&page_size=9&page=${page}`;
        const urlFinal = cleanedArgument ? `${urlBase}&search=${cleanedArgument}` : urlBase;
    ...
    function renderGamesBatch(games) {
        return Promise.all(games.map(game => fetchGameDetail(game.id)))
            .then(detailedGames => {
                const resultsContent = detailedGames.map(game => (
                    ...
                    <div class="card" data-id="${game.id}">
                        ...
                        <div class="extra-info">
                            ...
                            <h2>Type: ${game.genres?.map(pub => pub.name).join(', ') || "N/A"}</h2>
                ...
                // Appelle l’observer après chaque batch rendu
                setupCardObserver();
            });
    }
    ...
    const preparePage = () => {
        const resultsContainer = document.querySelector('.page-list .games');
        resultsContainer.innerHTML = ''; // Vide avant de commencer
